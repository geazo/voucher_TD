            ...
            <li class="nav-item">
                <a class="nav-link py-3 {{ $type == 'payment' ? 'active fw-bold text-success border-bottom border-success border-3' : 'text-muted' }}"
                   href="{{ route('admin.transactions', ['type' => 'payment']) }}">
                    <i class="bi bi-cart-check me-2"></i> Riwayat Pembayaran / Bermain
                </a>
            </li>
        </ul>
    </div>
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge {{ $t->wallet->type == 'Uang' ? 'bg-info' : 'bg-warning text-dark' }}">
                                {{ $t->wallet->type }}
                            </span>
                        </td>
                        <td class="fw-bold {{ $t->type == 'kredit' ? 'text-success' : 'text-danger' }}">
                            {{ $t->type == 'kredit' ? '+' : '-' }}
                            {{ $t->wallet->type == 'Uang' ? 'Rp ' : '' }}{{ number_format($t->nominal, 0, ',', '.') }}
                        </td>
                        <td>
                            <span class="badge {{ $t->type == 'kredit' ? 'bg-success' : 'bg-danger' }}">
                                {{ strtoupper($t->type) }}
                            </span>
                        </td>
                        <td>
                            <div class="small">{{ $t->operator->nama ?? 'System' }}</div>
                            <small class="text-muted" style="font-size: 0.75rem;">{{ strtoupper($t->operator->role ?? '') }}</small>
                        </td>
                    </tr>
                    @empty
                    <tr>
