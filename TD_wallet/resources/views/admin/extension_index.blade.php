                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                Rp {{ number_format($ext->transaction->sisa_saldo, 0, ',', '.') }}<br>
                                <small class="text-danger">Exp: {{ \Carbon\Carbon::parse($ext->transaction->expired_at)->format('d M Y') }}</small>
                            </td>
                            ...
                            <td class="text-center">
                                @if($ext->status == 'pending')
                                    <div class="d-flex justify-content-center gap-1">
                                        <form action="{{ route('superadmin.extensions.process', $ext->id) }}" method="POST" onsubmit="return confirm('Yakin setujui perpanjangan ini?');">
                                            @csrf
                                            <input type="hidden" name="action" value="approve">
                                            <button type="submit" class="btn btn-sm btn-success" title="Setujui"><i class="bi bi-check-lg"></i></button>
                                        </form>

                                        <form action="{{ route('superadmin.extensions.process', $ext->id) }}" method="POST" onsubmit="return confirm('Yakin tolak permintaan ini?');">
                                            @csrf
                                ...
                                @else
                                    <small class="text-muted">Diproses oleh:<br>{{ $ext->operator->nama ?? 'Admin' }}</small>
                                @endif
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="7" class="text-center py-4 text-muted">Belum ada pengajuan perpanjangan saldo saat ini.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
