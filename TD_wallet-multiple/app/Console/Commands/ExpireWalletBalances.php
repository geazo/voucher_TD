...
class ExpireWalletBalances extends Command
{
    ...
    public function handle()
    {
        ...
        // Cari topup yang masih punya sisa saldo dan sudah lewat batas waktunya
        ...
        try {
            foreach ($kreditHangus as $kredit) {
                $uangHangus = $kredit->sisa_saldo;

                // 1. Buat transaksi debit sebagai catatan uang hangus
                // Karena arsitektur Anda berbasis riwayat (SUM), membuat debit otomatis mengurangi saldo akhir.
                // Di dalam looping foreach
                Transaction::create([
                    'wallet_id'   => $kredit->wallet_id,
                    'type'        => 'adjustment', // <--- UBAH DI SINI
                    'nominal'     => $uangHangus,
                    'operator_id' => 1,
                    'keterangan'  => 'Expired: Saldo kedaluwarsa dari Topup tanggal ' . $kredit->created_at->format('d-m-Y')
                ]);

                // 2. Nol-kan sisa_saldo di data topup aslinya agar tidak dieksekusi lagi besok
