...
@push('styles')
    <style>
        ...
        .wallet-carousel .card-wrapper {
            ...
            margin: 0 5%; /* Memberi jarak agar kartu sebelah terlihat sedikit */
    ...
    <div class="container py-4">
        ...
        <div class="wallet-carousel mx-n3" id="walletCarousel">
            @forelse($displayCards as $index => $card)
                ...
                <div class="card-wrapper {{ $index === 0 ? 'active-card' : '' }}" data-membership-id="{{ $card->membership_id }}">
                    <a href="{{ route('customer.saldo.info') }}?membership_id={{ $card->membership_id }}" class="text-decoration-none">
                                ...
                                <div class="row">
                                    <div class="col-6 border-end border-dark border-opacity-10">
                                        ...
                                        <div class="fw-bold text-nowrap" style="font-size: clamp(1.1rem, 5vw, 1.5rem);">
                                            <span style="font-size: 0.7em;">Rp</span> {{ number_format($card->saldoUang, 0, ',', '.') }}
                                        </div>
                                    </div>
                                    <div class="col-6 ps-3">
                                        <p class="mb-1 opacity-75" style="font-size: 0.8rem;">Saldo Poin</p>
                                        <div class="fw-bold text-nowrap" style="font-size: clamp(1.1rem, 5vw, 1.5rem);">
                                            {{ number_format($card->saldoPoin, 0, ',', '.') }} <span style="font-size: 0.7em;" class="opacity-75">Pts</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="card-body p-0">
                <div class="list-group list-group-flush" id="transactionList">
                    @foreach($transactions as $t)
                        @php
                            if ($t->type === 'kredit') {
                                $judul = 'Topup Saldo'; $icon = 'bi-wallet2 text-success'; $tanda = '+'; $warna = 'text-success';
                            } elseif ($t->type === 'adjustment') {
                                $judul = 'Expired / Hangus'; $icon = 'bi-calendar-x text-warning'; $tanda = '-'; $warna = 'text-warning';
                            } else {
                                $judul = 'Pembayaran'; $icon = 'bi-cart-dash text-danger'; $tanda = '-'; $warna = 'text-danger';
                            }
                        ...
                        <a href="{{ route('customer.transaction.detail', $t->id) }}"
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex align-items-center gap-3">
                                    <div class="bg-light rounded-circle d-flex justify-content-center align-items-center" style="width: 42px; height: 42px;">
                                ...
                                <div class="text-end">
                                    <p class="mb-0 fw-bold {{ $warna }}">{{ $tanda }} {{ number_format($t->total_nominal, 0, ',', '.') }}</p>
                                    <span class="badge bg-light text-muted fw-normal border" style="font-size: 0.65rem;">{{ strtoupper($t->type) }}</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
